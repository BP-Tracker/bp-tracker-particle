(function($) {
  ...
  // NOTE: markdown filenames cannot contain hyphens
  var hash = document.location.hash;
  var docFile = hash !== "" ? hash.replace("#", "").split('-') : ["guide"];

  var fetcher = Flatdoc.github('BP-Tracker/bp-tracker-particle', 'docs/src/content/'
        + docFile[0] + '.md');

  if(document.location.hostname === "localhost"){
    fetcher = Flatdoc.file("./src/content/" + docFile[0] + ".md");
  }
  Flatdoc.run({ fetcher: fetcher });

  //TODO: not working ATM
  $(".root-doc").on('click', function(){
    console.log('click');
    Flatdoc.run({ fetcher: Flatdoc.file("./src/content/reference.md") });
  });
